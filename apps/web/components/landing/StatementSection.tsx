...
const TOKENS: Tok[] = [
  ...
  { w: "your company's context", chip: { icon: "M12 3 4 6v5c0 5 3.4 8.3 8 10 4.6-1.7 8-5 8-10V6z", color: "var(--l-ink)" } },
  { w: "," }, { w: "enforces" }, { w: "your" }, { w: "standards," }, { w: "remembers" },
  { w: "past decisions", chip: { icon: "M12 8v4l3 2 M12 3a9 9 0 1 0 9 9", color: "var(--l-ink)" } },
  { w: "," }, { w: "and" }, { w: "stops" }, { w: "mistakes" },
  { w: "before they ship", chip: { icon: "M5 12h14 M13 6l6 6-6 6", color: "var(--l-ink)" } },
  { w: "." },
];
...
export function StatementSection() {
  ...
  return (
    <section ref={ref} id="statement" style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: "150px 24px 130px", textAlign: "center" }}>
      <div style={{ fontFamily: "var(--font-mono), monospace", fontSize: 12, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--l-muted2)", marginBottom: 30 }}>The idea</div>
      <p style={{ fontFamily: "var(--font-display), sans-serif", fontWeight: 500, fontSize: "clamp(26px, 4vw, 44px)", lineHeight: 1.28, letterSpacing: "-0.02em", margin: 0 }}>
        {TOKENS.map((t, i) => {
          ...
          if (t.chip) {
            return (
              <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 8, whiteSpace: "nowrap", padding: "2px 12px 2px 10px", margin: "0 4px", borderRadius: 99, border: `1px solid ${on ? t.chip.color : "var(--l-border)"}`, background: on ? `color-mix(in srgb, ${t.chip.color} 9%, transparent)` : "transparent", color: on ? "var(--l-ink)" : "var(--l-word-off)", transition: "color .35s ease, background-color .35s ease, border-color .35s ease", verticalAlign: "middle" }}>
                <motion.svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={t.chip.color} strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" animate={{ scale: on ? 1 : 0 }} transition={{ duration: 0.45, ease: [0.2, 1.5, 0.4, 1] }} style={{ borderRadius: 99 }}>
